        ...
        <?php include 'header.php'; ?>
            ...
            <div class="container">
                ...
                if ($result && $result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $items[] = [
                            "type"      => "gallery",
                            "id"        => "g_" . $row['idy'],
                            "title"     => $row['event-date'] . " " . $row['pastevent-name'],
                            "detail"    => $row["past-event-full-detail"],
                            "images"    => explode(",", $row["yearwise_images"]),
                            "titles"    => explode("|", $row["pastevent_image_content"]),
                            "sort_date" => $row['event-date'],
                        ];
                    }
                }
                ...
                $syncStmt = $conn->prepare(
                    "SELECT * FROM sync_event WHERE status = 'PUBLISHED' AND start_date < NOW() AND YEAR(start_date) = ? ORDER BY start_date DESC"
                );
                $syncStmt->bind_param("i", $year);
                $syncStmt->execute();
                ...
                if ($syncResult && $syncResult->num_rows > 0) {
                    while ($row = $syncResult->fetch_assoc()) {
                        ...
                        if (is_array($rawGallery)) {
                            foreach ($rawGallery as $g) {
                                if (is_string($g) && trim($g) !== '') {
                                    ...
                                    $candidate = $g['url'] ?? $g['src'] ?? $g['link'] ?? null;
                                    if ($candidate) $galleryImgs[] = $candidate;
                                }
                            }
                        }

                        if (empty($galleryImgs) && !empty($row['image'])) {
                        ...
                        $items[] = [
                            "type"      => "synced",
                            "id"        => "s_" . $row['id'],
                            "title"     => date("d M Y", strtotime($row['start_date'])) . " " . $row['name'],
                            "detail"    => $row['location'],
                            "images"    => $galleryImgs,
                            "titles"    => array_fill(0, count($galleryImgs), $row['name']),
                            "sort_date" => $row['start_date'],
                        ];
                    }
                }

                // Dono ko date ke hisaab se sort kar do (latest upar)
                ...
                <div class="accordion-item">
                    ...
                    <div id="acc_<?php echo $idy; ?>" class="accordion-body">
                         ...
                         <div id="banner">
                            <div id="inner-banner"> 
                                <h1><?php echo htmlspecialchars($item['detail']); ?></h1>
                            </div>
                        </div>
                        ...
                        <div class="row" style="margin-left: 5px;">
                            <?php foreach ($item['images'] as $i => $img) { 
                                ...
                                if ($item['type'] === 'synced') {
                                    // Synced event ka image already poora URL hota hai
                                    $imgSrc = $img;
                                    $imgHref = $img;
                                } else {
                                    $imgSrc = "https://smedatabank.com/pastevent-image/" . $img . ".JPG";
                                    $imgHref = "images/pastevent-big-images/" . $img . ".JPG";
                                }
                            ?>
                                <div class="span3">
                                    <a data-fancybox="gallery"
                                       data-caption="<?php echo htmlspecialchars($item['titles'][$i] ?? ''); ?>"
                                       href="<?php echo $imgHref; ?>">

                                        <img class="rounded" src="<?php echo $imgSrc; ?>" title="<?php echo htmlspecialchars($item['titles'][$i] ?? ''); ?>">
                                    </a>
                                </div>
                            <?php } ?>
                        </div>
