...
if ($err) {
  $errMSGs = "Error :" . $err;
	...
	if($httpcode == 200){
        ...
        $businessactivity = $_SESSION["businessactivity"];
		...
		$sql = "INSERT INTO SME_Consultants(title, fname, lname, designation, companyname, businessactivity, 
	        businesssector, businessinterest, service, sector, email, mobilenumber, phonenumber, address)
            VALUES ('$title', '$fname','$lname','$designation','$companyname','$businessactivity','$businesssector',
		    '$businessinterest', '$service', '$sector', '$email', '$mobilenumber', '$phonenumber', '$address')";
...
# FIX: Replace this email with recipient email
		# Sender Data
        $subject = "SME Consultants | SME Chamber of India";
        ...
        $businessactivity = trim($_SESSION["businessactivity"]);
        $businesssector = trim($_SESSION["businesssector"]);
        $businessinterest = trim($_SESSION["businessinterest"]);
        $service = trim($_SESSION["service"]);
        $sector = trim($_SESSION["sector"]);
        ...
        $content .= "Business Activity: $businessactivity \n\n";
        $content .= "Business Sector: $businesssector \n\n";
        $content .= "Business Interest: $businessinterest \n\n";
        $content .= "Details of Consultancy Services to promote amongst SMEs: $service \n\n";
        $content .= "Business Sectors of SMEs: $sector \n\n";
		
        # email headers.
        $headers = "From: SME Chamber of India <no-reply@smechamberofindia.com>" . "\r\n";
        $headers .= "To: $title $fname $lname <$email>" . "\r\n";
		$headers .= "Cc: secretariat@smechamber.com" . "\r\n";
        $headers .= "Bcc: gandhi@smechamber.in" . "\r\n";
		//$headers .= "Bcc: rashmi.smechamber@gmail.com" . "\r\n";
        ...
        if ($success) {
            ...
            $_SESSION["successMSG"] = "Thank you for showing interest as SME Consultant. We will send the proposal shortly.";
                                  ...
                                  <p>Fields marked with a <span class="emp">*</span> are required.</p><br>
									<form action="" method="post">
									  <div class="form-group">
										...
										<span>Did not receive OTP? <a href="form-verification-consultant.php?do=resend">Resend</a></span>
									  </div>
