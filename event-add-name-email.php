...
$title = $fname = $lname= $designation = $companyname = $businessactivity = $businesssector = $businessinterest = $lookingsupport = $phonenumber = $mobilenumber = $email = $address = $captcha = "";
$to = "director@smechamber.com";
$sub = "Request to  Add name For Events";
...
if(isset($_POST['lookingsupport']))
$lookingsupport=$_POST['lookingsupport'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST['businessactivity'])) {
      ...
      if (empty($_POST['lookingsupport'])) {
        $error = "Please Select Any Option";
    } else {
        $businesssupport = $_POST['lookingsupport'];
    }
...
$fnlmsg1 = "$title $fname $lname $designation $companyname $businessactivity $businesssector $businessinterest $lookingsupport $phonenumber $mobilenumber $email $address";
...
ob_start();
include './event-composemsg.php';
$message = ob_get_clean();
if (empty($error)) {

// mail($to, $sub, " Name : $name\n E-mail : $email\n Message : $msg");
    $headers = 'MIME-Version: 1.0' . "\r\n";
    $headers = 'From: Sme Chamber of India For Event Information' . "\r\n";
    ...
    echo "<script type='text/javascript'>alert('$error'); window.location.href='event-form.php';</script>";
}
?>